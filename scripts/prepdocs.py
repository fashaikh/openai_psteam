...
def blob_name_from_file_page(filename, page = 0):
    """Generate a blob name from a filename and page number"""
...
def upload_blobs(filename):
    """Upload a file to Azure Blob Storage, splitting into pages if it's a PDF"""
...
def remove_blobs(filename):
    """Remove blobs for a file from Azure Blob Storage"""
...
def get_document_text(filename):
    """Extract text from a file using Azure Form Recognizer or local pypdf parser"""
    offset = 0
    page_map = []
...
def split_text(page_map):
    """Split text into sections of MAX_SECTION_LENGTH characters, trying to find the end of the sentence"""
    SENTENCE_ENDINGS = [".", "!", "?"]
    WORDS_BREAKS = [",", ";", ":", " ", "(", ")", "[", "]", "{", "}", "\t", "\n"]
...
def create_sections(filename, page_map):
    """Create sections from the page map"""
...
def index_sections(filename, sections):
    """Index the sections into the search index"""
